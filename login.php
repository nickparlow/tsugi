...
if ( $key_row === false ) {
    die_with_error_log('Error: No key defined for LDAP authentication domain: ' . $oauth_consumer_key);
}
$ldap_key_id = $key_row['key_id']+0;
$ldap_secret = $key_row['secret'];
if ( $ldap_key_id < 1 ) {
    die_with_error_log('Error: No key for LDAP authentication domain: ' . $oauth_consumer_key);
}
